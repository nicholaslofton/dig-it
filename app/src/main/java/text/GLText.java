...
import text.programs.BatchTextProgram;
import text.programs.Program;
...
public class GLText {
	...
	public void draw(String text, float x, float y, float z, float angleDegX, float angleDegY, float angleDegZ)  {
		...
		for (int i = 0; i < len; i++)  {              // FOR Each Character in String
			...
			if (lineWrap > 0 && letterX + cellWidth > lineWrap )  {
				letterX = 0;                                            // Reset X Position to Start
				letterY -= chrHeight; // Move to Next Row (Cell)
			}
		}
	}
	...
	//--Set Line Wrap--//
	public void setLineWrap(float bounding)  {
		lineWrap = bounding;                        // Set Uniform Scale
	}

	//--Set Line Height--//
	public void setLineHeight(float bounding)  {
		lineHeight = bounding;                        // Set Uniform Scale
	}
